...
#define 	baud	100000
#define     ADDRESS_DS1307  0b1101000
#define     ADDRESS_EEPROM  0b1010000
...
void ds1307_control(unsigned char mRW_bit)
{
    ...
    SSPBUF = (ADDRESS_DS1307 << 1) + mRW_bit;  // send the control byte
...
{
    PIR1bits.SSPIF=0;          // clear SSP interrupt bit
    SSPBUF = (ADDRESS_EEPROM << 1) + mRW_bit;  // send the control byte
    while(!PIR1bits.SSPIF);    // Wait for interrupt flag to go high indicating transmission is complete
}
